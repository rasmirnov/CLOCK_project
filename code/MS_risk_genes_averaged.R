...
seurat_subset <- subset(seurat_subset, time %in% c('BSE', 'W5_10', 'YR1_2'))
cell_type_list <- c('CD4 Naive', 'CD4 Treg naive', 'CD4 Treg memory', 'CD4 Th1', 'CD4 Th17', 'CD4 Th22', 'CD4 Temra',
                    'CD4 exhausted') # remove only for CSF-W5_10: 'B Transitional', 
...
pbulk <- AverageExpression(
  object = seurat_subset,
  assays = 'RNA',
  features = NULL,
  return.seurat = TRUE,
  group.by = c('cell_type.time'),  
  add.ident = NULL,
  slot = "counts",
  verbose = TRUE)
...
# 8.2. normilize (for DE)
pbulk <- NormalizeData(pbulk, normalization.method = "LogNormalize",
                       scale.factor = 10000)

# 8.3. scale (for heatmap): IMPORTANT!
all.genes <- rownames(pbulk)
pbulk <- ScaleData(pbulk, features = all.genes)

# 8.4. modify data-2: 2nd line is optional
pbulk@meta.data$cell_type.time <- rownames(pbulk@meta.data)
...
pbulk$cell_type.time <- factor(pbulk$cell_type.time,
                               levels = c("CD4 Naive_BSE",         "CD4 Naive_W5_10",       "CD4 Naive_YR1_2",
...
# 17. choose colors
mycolors <- c('#d60000',  '#ff7266', '#edb8b8', '#e36414', '#ff9f1c', '#ffbf69', '#f9cb9cff', 
              '#fdf490')
...
# 20. save
path_2 <- '/storage1/fs1/martyomov/Active/collaborations/romans/Greg_wu/CLOCK/output/figures/pdf/DEGs_risk_genes/averaged_expr/'
# install.packages('tidyHeatmap')
# library(tidyHeatmap)
save_pdf(p, paste0(path_2, "heatmap_", tissue_s, "_CD4.pdf"),
         width = 7, height = 7)
...
###----------------------------Using DoHeatmap----------------------------------------------------
# 8.3. add metadata
levels(pbulk) <- c("B Atypical_BSE",    "B Switched_BSE",   
                   "B Naive_BSE",       "Plasmablast_BSE",  
                   "Plasmablast_W5_10", "B Naive_W5_10",    
                   "B Switched_W5_10",  "B Atypical_W5_10")

# 8.4. reassign Idents to "cell_type.organ" (for DE: comparation)
...
### 9. load all DEGs per tissue-treatment group 
all_degs <- read.csv('DEA/CSF_W5_10/combined.csv')
ms_risk_genes <- read.csv('MS_risk_genes/GWAS_MS_risk_genes.csv')
...
sub_degs <- all_degs %>% 
  filter(gene %in% ms_risk_genes$risk_genes) %>% 
  filter(p_val < 0.05) %>%      # filter(avg_log2FC > 0.25 | avg_log2FC < -0.25)
  filter(cell_type %in% cell_type_list)
...
mapal <- colorRampPalette(RColorBrewer::brewer.pal(11,"RdBu"))(256)
DoHeatmap(pbulk, 
          features = sub_degs$gene,
          group.by = "cell_type.time",
          slot = 'scale.data',
          size = 5,    
          angle = 45,
          draw.lines = F) +
  scale_fill_gradientn(colours = rev(mapal)) +   # scale_fill_viridis(option = 'inferno') +
  guides(color = FALSE) +                   # hide cell_type legend
  theme(text = element_text(size = 12))

# 13. save
ggsave(paste0(path, "output/output/MS_risk_genes/averaged_expr/CSF_B_w5_10.png"), 
       dpi = 300, 
       width = 6, 
       height = 9)
###----------------------------------------------------------------------------------------------------------
